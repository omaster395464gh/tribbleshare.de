:source-highlighter: highlight.js

== tribbleshare.de

=== Voraussetzungen
* *Ionos Server VPS L (6 CPU, 8 GB RAM)* / Debian 13 (initial Debian 12)
* Tailscale VPN

=== Vorbereitung

[source,bash]
----
adduser admin
usermod -aG sudo admin
echo "%sudo ALL=(ALL) NOPASSWD: ALL" >/etc/sudoers.d/custom-admin
echo 'Defaults:%sudo env_keep += "http_proxy https_proxy ftp_proxy all_proxy no_proxy SSH_AUTH_SOCK RESTIC_PASSWORD"' >>/etc/sudoers.d/custom-admin
curl -fsSL https://tailscale.com/install.sh | sh
tailscale up
----

=== Software-Installation (admin) and docker run
[source,bash]
----
sudo apt install git git-lfs docker docker-compose rsync restic nfs-common
git pull https://github.com/omaster395464gh/tribbleshare.de.git
cd ~/tribbleshare.de/server
# Allow docker without sudo
sudo usermod -aG docker $USER
sudo docker-compose up -d
# TLS Cert
sudo tailscale cert \
  --cert-file /home/admin/certs/tribbleshare.follow-inconnu.ts.net.crt \
  --key-file /home/admin/certs/tribbleshare.follow-inconnu.ts.net.key \
  tribbleshare.follow-inconnu.ts.net
----

=== Upgrade / Software-Wartung

[source,bash]
.~/tribbleshare.de/scripts/1-do-upgrade.sh (Einrichtung als Dienst: lesen und Anweisungen in den Kommentaren ausführen)
----
bash ~/tribbleshare.de/scripts/1-do-upgrade.sh
----

=== Reboot automatisieren

[source,bash]
./etc/apt/apt.conf.d/50unattended-upgrades
----
Unattended-Upgrade::Remove-Unused-Kernel-Packages "true";
Unattended-Upgrade::Remove-New-Unused-Dependencies "true";
Unattended-Upgrade::Remove-Unused-Dependencies "true";
Unattended-Upgrade::Automatic-Reboot "true";
Unattended-Upgrade::Automatic-Reboot-WithUsers "false";
Unattended-Upgrade::Automatic-Reboot-Time "02:00";
----

=== Troubleshooting / Neustart des Servers

==== SSH (wenn Server noch läuft)
* Windows: PageAnt starten, Key laden
* Login per SSH an admin@tribbleshare.follow-inconnu.ts.net (Windows per Putty, Linux per Terminal)
[source,bash]
----
ssh admin@tribbleshare.follow-inconnu.ts.net
sudo shutdown -r now
----

==== Ionos Server Console (wenn nix mehr geht)
* Website https://www.ionos.de
* Login 33*. (Passwort + 2FA)
* Option "Server & Cloud"
* Server: "My VPS" markieren
* Aktionen "Neu starten"

=== Troubleshooting / Reboot automatisieren / Server hängt (Schreiben auf Terminal)

[source,bash]
./etc/grub.conf ergänzen
----
GRUB_CMDLINE_LINUX_DEFAULT="quiet systemd.show_status=no nomodeset"
# GRUB_CMDLINE_LINUX_DEFAULT=""
----
dann: 
update-grub

[source,bash]
./etc/systemd/system.conf ergänzen
----
DefaultTimeoutStopSec=30s
----

=== Troubleshooting / NFS nassaja einhängen
[source,bash]
./etc/fstab ergänzen
----
nassaja:/volume1/Multimedia  /Multimedia  nfs  noauto,x-systemd.automount,x-systemd.idle-timeout=600,_netdev,nofail,noatime,tcp,actimeo=1800  0  0
----


=== Backup / Restic

[source,bash]
.~/tribbleshare.de/scripts/2-do-backup.sh
----
# restic vorbereiten / tribbleshare
# Nutzer ts_backup anlegen
# RESTIC_PASSWORD setzen in /etc/profile.d/restic.sh
sftp ts_backup@box3 <<EOF
pwd
EOF
# Initialisieren
sudo restic -r sftp:ts_backup@box3:/F:/Backups/ts_backup init

# Backup ausführen
sudo restic -r sftp:ts_backup@box3:/F:/Backups/ts_backup backup /home/admin

----
https://restic.net/
